...
// Galería "De la bobina al producto convertido" — pedido explícito del
// cliente (feedback de voz, 2026-09-17): a los íconos de línea les
// faltaba algo de foto real intercalada. En vez de una foto por ícono
// (obligaría a inventar/forzar una para cada uno), la opción elegida es
// la que propuso el cliente: una tira de fotos deslizable aparte, que
// recorra el proceso completo bobina → conversión → producto terminado.
// Sólo 7 fotos (el cliente tiró "10, 15, lo que sea" como número al
// voleo, no un requisito) — son las únicas fotos reales de este tramo
// que subió el cliente sin repetir ninguna foto ya usada en otra sección
// del sitio (mismo criterio de "ninguna foto se repite" del resto del
// sitio). Orden intencional: depósito → logística → conversión →
...
const GALLERY_PHOTOS = [
  ...
  { src: "/photos/papel-tissue-produccion-operarios.png", alt: "Operarios controlando línea de conversión de papel Tissue" },
    ...
    <section id="papel" className="relative w-full bg-paper">
      ...
      <div ref={catalogRef} className="container-industrial py-16 md:py-24">
        <div className="border-t border-line-on-light pt-14 md:pt-20">
          ...
          {/* Galería "De la bobina al producto convertido" — ver
              GALLERY_PHOTOS arriba. Tira horizontal deslizable en TODOS los
              breakpoints (no sólo mobile): a diferencia de ProductFamilies/
              PapelTissueSpecs de arriba, acá no hay un layout de grilla
              alternativo en desktop — el cliente la pidió explícitamente
              como algo para "desplazarte", no para ver todo de una. Mismas
              flechas prev/next que ya se usaban en el carrusel mobile de
              Proceso Industrial (sección eliminada, pero el patrón de
              affordance para desktop-sin-trackpad sigue siendo válido acá). */}
